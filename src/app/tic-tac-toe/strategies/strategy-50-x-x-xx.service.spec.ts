...
import { testCases } from './testCases/test_50-x-x-xx'
...
let testedFunction = function(figureToFind:FigureNotEmpty){
  return function(singleTestCase:TestCase, nrOfFiguresInRowToWinn: number){
    it(singleTestCase.name, () => {
      ...
      let sollution = patternSearcher.getCalculatedStrategy(figureToFind, 'strategy:X-X-XX');
...
runTestSuit(testedFunction('Circle'), 'Find Strategy_50_X_X_XX_Service pattern: circle test instances', testCases);