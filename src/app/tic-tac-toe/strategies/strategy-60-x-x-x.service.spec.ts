...
import { testCases } from './testCases/test_60-x-x-x'
...
import { Strategy60_X_X_XService } from './strategy-60-x-x-x.service';
...
let testedFunction = function(figureToFind:FigureNotEmpty){
  return function(singleTestCase:TestCase, nrOfFiguresInRowToWinn: number){
    it(singleTestCase.name, () => {
      ...
      let sollution = patternSearcher.getCalculatedStrategy(figureToFind, 'strategy:X-X-X');
